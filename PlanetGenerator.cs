...
{
    [Export]
    public PlanetSettings Settings { get; set; } = new();
    ...
    public void GeneratePlanet()
    {
        ...
        IcosphereData icosphere = BuildIcosphere(Mathf.Max(0, Settings.Subdivisions));
        LandmassPreset landmassPreset = ResolveLandmassPreset(Settings);
        NoiseField terrainNoise = BuildNoise(Settings.TerrainSeed, Settings.NoiseScale, 5, 2.0f, 0.5f);
        NoiseField continentNoise = BuildNoise(Settings.ContinentSeed, landmassPreset.ContinentNoiseScale, 3, 2.0f, 0.55f);
        NoiseField continentWarpX = BuildNoise(Settings.ContinentSeed + 1001, landmassPreset.ContinentWarpScale, 2, 2.0f, 0.5f);
        NoiseField continentWarpY = BuildNoise(Settings.ContinentSeed + 2003, landmassPreset.ContinentWarpScale, 2, 2.0f, 0.5f);
        NoiseField continentWarpZ = BuildNoise(Settings.ContinentSeed + 3001, landmassPreset.ContinentWarpScale, 2, 2.0f, 0.5f);
        NoiseField mountainNoise = BuildNoise(Settings.MountainSeed, Settings.MountainNoiseScale, 5, 2.0f, 0.5f);
        ...
        BiomeThresholds thresholds = CalculateBiomeThresholds(Settings);
        ...
        for (int i = 0; i < icosphere.Vertices.Count; i++)
        {
            ...
            displacedVertices[i] = direction * (Settings.Radius + heightOffset);
    ...
    private NoiseField BuildNoise(int seed, float scale, int octaves, float lacunarity, float gain)
    {
        ...
        return new NoiseField(noise, Settings.Radius);
    }
    ...
    private static BiomeThresholds CalculateBiomeThresholds(PlanetSettings settings)
    {
        ...
        float mountainRatio = settings.MountainPercent / total;
        ...
        return new BiomeThresholds(
            ...
            mountains: mountainRatio <= 0.0f ? 1.0f : 1.0f);
    }
    ...
    private float ResolveHeightOffset(PlanetBiome biome, float terrainValue, float mountainValue)
    {
        return biome switch
        {
            PlanetBiome.Water => terrainValue * Settings.WaterHeight,
            PlanetBiome.Wetlands => Mathf.Lerp(Settings.WaterHeight, Settings.WetlandsHeight, terrainValue),
            PlanetBiome.Plains => Mathf.Lerp(Settings.WetlandsHeight, Settings.PlainsHeight, terrainValue),
            PlanetBiome.Forest => Mathf.Lerp(Settings.PlainsHeight, Settings.ForestHeight, terrainValue),
            PlanetBiome.Desert => Mathf.Lerp(Settings.WetlandsHeight, Settings.DesertHeight, terrainValue),
            PlanetBiome.Mountains => Mathf.Lerp(Settings.ForestHeight, Settings.MountainHeight, mountainValue),
            _ => 0.0f
        };
    }
    ...
    private Color ResolveColor(PlanetBiome biome, float terrainValue, float mountainValue)
    {
        return biome switch
        {
            PlanetBiome.Water => Settings.WaterColor.Darkened((1.0f - terrainValue) * 0.2f),
            PlanetBiome.Wetlands => Settings.WetlandsColor.Darkened((1.0f - terrainValue) * 0.1f),
            PlanetBiome.Plains => Settings.PlainsColor.Lightened((terrainValue - 0.45f) * 0.12f),
            PlanetBiome.Forest => Settings.ForestColor.Darkened((1.0f - terrainValue) * 0.14f),
            PlanetBiome.Desert => Settings.DesertColor.Lightened(terrainValue * 0.14f),
            PlanetBiome.Mountains => Settings.MountainColor.Lightened(mountainValue * 0.2f),
            _ => Colors.White
        };
    }
    ...
    {
        const string collisionNodeName = "PlanetCollision";

        if (HasNode(collisionNodeName))
        {
            GetNode<StaticBody3D>(collisionNodeName).QueueFree();
        }
    ...
    {
        public BiomeThresholds(float water, float wetlands, float plains, float forest, float desert, float mountains)
        {
            Water = water;
            Wetlands = wetlands;
            Plains = plains;
